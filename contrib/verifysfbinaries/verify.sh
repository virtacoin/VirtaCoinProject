...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'virtacoin-',
...
if [ $? -ne 0 ]; then
   ...
   echo "[virtacoin-]<version>-[rc[0-9]] (example: virtacoin-0.7.1-rc1)"
   echo "wget output:"
