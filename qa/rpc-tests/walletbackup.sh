...
B4ARGS="-datadir=$D4"
$VIRTACOIND $VIRTACOINDARGS $B4ARGS &
B4PID=$!
...
B3ARGS="-datadir=$D3"
$VIRTACOIND $VIRTACOINDARGS $B3ARGS &
B3PID=$!
...
function S {
  ...
  if [[ $TXID == "" ]] ; then
      echoerr "node${1}: error sending ${3} vta"
...
function StartThree {
  $VIRTACOIND $VIRTACOINDARGS $B1ARGS &
  B1PID=$!
  $VIRTACOIND $VIRTACOINDARGS $B2ARGS &
  B2PID=$!
  $VIRTACOIND $VIRTACOINDARGS $B3ARGS &
  B3PID=$!
}
