...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or virtacoin.conf setting gen)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    if (fGenerate && Params().NetworkID() == CChainParams::REGTEST)
    {
        ...
        while (nHeight < nHeightEnd)
        {
            ...
            {
                nHeightLast = nHeight;
                GenerateVirtaCoins(fGenerate, pwalletMain, 1);
            }
            MilliSleep(1);
            {   // Don't keep cs_main locked
                LOCK(cs_main);
    ...
    else // Not -regtest: start generate thread, return immediately
    {
        ...
        GenerateVirtaCoins(fGenerate, pwalletMain, nGenProcLimit);
    }
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "VirtaCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "VirtaCoin is downloading blocks...");
