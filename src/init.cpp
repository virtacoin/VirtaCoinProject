...
void Shutdown()
{
    ...
    RenameThread("virtacoin-shutoff");
    ...
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateVirtaCoins(false, NULL, 0);
#endif
    StopNode();
...
std::string HelpMessage(HelpMessageMode hmm)
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: virtacoin.conf)") + "\n";
    strUsage += "  -datadir=<dir>         " + _("Specify data directory") + "\n";
    strUsage += "  -testnet               " + _("Use the test network") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: virtacoind.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 22816 or testnet: 18333)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 22815 or testnet: 18332)") + "\n";
    ...
    strUsage += "\n" + _("SSL options: (see the VirtaCoin Wiki for SSL setup instructions)") + "\n";
...
{
    RenameThread("virtacoin-loadblk");
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single VirtaCoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. VirtaCoin is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("VirtaCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of VirtaCoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart VirtaCoin to complete") << "\n";
    ...
    if (pwalletMain)
        GenerateVirtaCoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", -1));
#endif
