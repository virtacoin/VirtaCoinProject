...
const int VIRTACOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString VIRTACOIN_IPC_PREFIX("virtacoin:");
const char* VIRTACOIN_REQUEST_MIMETYPE = "application/virtacoin-paymentrequest";
const char* VIRTACOIN_PAYMENTACK_MIMETYPE = "application/virtacoin-paymentack";
const char* VIRTACOIN_PAYMENTACK_CONTENTTYPE = "application/virtacoin-payment";
...
bool PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(VIRTACOIN_IPC_PREFIX, Qt::CaseInsensitive)) // virtacoin: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseVirtaCoinURI(arg, &r))
            {
                CVirtaCoinAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    foreach (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(VIRTACOIN_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            return false;
        }
        ...
        socket->waitForBytesWritten(VIRTACOIN_IPC_CONNECT_TIMEOUT);
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click virtacoin: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start virtacoin: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling virtacoin: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject *object, QEvent *event)
{
    // clicking on virtacoin: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in virtacoin: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(VIRTACOIN_IPC_PREFIX, Qt::CaseInsensitive)) // virtacoin: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseVirtaCoinURI(s, &recipient))
                emit receivedPaymentRequest(recipient);
            else
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid VirtaCoin address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach(const PAIRTYPE(CScript, qint64)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CVirtaCoinAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()){
            // Insecure payments to custom virtacoin addresses are not supported
        ...
        if (txOut.IsDust(CTransaction::nMinRelayTxFee)) {
            QString msg = tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(VirtaCoinUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second));
...
void PaymentServer::fetchRequest(const QUrl& url)
{
    ...
    netRequest.setRawHeader("Accept", VIRTACOIN_REQUEST_MIMETYPE);
...
void PaymentServer::fetchPaymentACK(CWallet* wallet, SendCoinsRecipient recipient, QByteArray transaction)
{
    ...
    netRequest.setHeader(QNetworkRequest::ContentTypeHeader, VIRTACOIN_PAYMENTACK_CONTENTTYPE);
    netRequest.setRawHeader("User-Agent", CLIENT_NAME.c_str());
    netRequest.setRawHeader("Accept", VIRTACOIN_PAYMENTACK_MIMETYPE);
