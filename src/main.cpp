...
#if defined(NDEBUG)
# error "VirtaCoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "VirtaCoin Signed Message:\n";
...
static const int64_t nDiffChangeTarget = 1000; // Patch effective @ block 1000
static const int64_t patchBlockRewardDuration = 10080; // 10080 blocks main net change

int64_t GetVTASubsidy(int nHeight) {
...
int64_t GetBlockValue(int nHeight, int64_t nFees) {
   ...
   if(nHeight < nDiffChangeTarget) {
     // reward 1 for 1000 blocks
     nSubsidy = 1;
     if(nHeight == 1) {
        // pre-mine to distribute to existing VirtaCoin user balances.
        nSubsidy = 8400000000 * COIN;
     }
   } else {
      // patch takes effect after 1000 blocks solved
      nSubsidy = GetVTASubsidy(nHeight);
   }
   //make sure the reward is at least 1 VTA
   if(nSubsidy < COIN) {
      nSubsidy = COIN;
   }
...
static const int64_t nTargetTimespan =  0.1 * 24 * 60 * 60; // 2.4 hours
static const int64_t nTargetSpacing = 60; // 60 seconds
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;
static const int64_t nTargetTimespanRe = 60; // 60 Seconds
static const int64_t nTargetSpacingRe = 60; // 60 seconds
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // VirtaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
void ThreadScriptCheck() {
    RenameThread("virtacoin-scriptch");
